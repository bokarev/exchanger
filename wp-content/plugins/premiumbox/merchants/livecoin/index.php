...
description: [ru_RU:]мерчант LiveCoin[:ru_RU][en_US:]LiveCoin merchant[:en_US]
version: 1.2
*/
						...
						if($code){
							...
							try{
								...
								if($info){
									...
									$livecoin_sum = is_isset($info,'amount');
									$livecoin_currency = strtoupper(is_isset($info,'currency'));
									$livecoin_trans_id = trim(is_isset($info,'id'));
									if($livecoin_sum >= $sum){
									
										if($livecoin_currency == $currency){
											
											$params = array(
												'pay_purse' => '',
												'sum' => $livecoin_sum,
												'naschet' => '',
												'trans_in' => $livecoin_trans_id,
											);
