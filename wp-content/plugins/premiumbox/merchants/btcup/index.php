...
description: [ru_RU:]мерчант BTC-UP[:ru_RU][en_US:]BTC-UP merchant[:en_US]
version: 1.2
*/
						...
						if($code){
							try{
								...
								if($info){
									...
									$btcup_sum = is_isset($info,'couponAmount');
									$btcup_currency = strtoupper(is_isset($info,'couponCurrency'));
									$btcup_trans_id = trim(is_isset($info,'transID'));
									if($btcup_sum >= $sum){
									
										if($btcup_currency == $currency){
											
											$params = array(
												'pay_purse' => '',
												'sum' => $btcup_sum,
												'naschet' => '',
												'trans_in' => $btcup_trans_id,
											);
